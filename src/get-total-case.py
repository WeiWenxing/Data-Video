...
def getArea(input):
    ...
    df = pd.read_csv(input, usecols=['countryEnglishName', 'provinceEnglishName', 'province_confirmedCount', 'updateTime'], parse_dates=['updateTime'], date_parser=dateparse)
    result1 = df[df["countryEnglishName"] == df["provinceEnglishName"]]
    result1.rename(columns={'countryEnglishName':'type', 'province_confirmedCount':'value'}, inplace=True)
    ...
    result1 = result1.drop_duplicates(subset=['type', 'date'], keep='first')
...
#    df = pd.read_csv(input, encoding="gbk", usecols=['type', 'value', 'date'])
    ...
    result1 = pd.read_csv(input, usecols=['confirmedCount', 'updateTime'], parse_dates=['updateTime'], date_parser=dateparse)
    result1.rename(columns={'confirmedCount':'value'}, inplace=True)
    result1['date'] = result1['updateTime'].dt.date
    result1 = result1.drop(columns=['updateTime'])
